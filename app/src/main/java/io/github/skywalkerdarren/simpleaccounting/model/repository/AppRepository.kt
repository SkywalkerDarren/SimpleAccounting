...
class AppRepository private constructor(val executors: AppExecutors, val database: AppDatabase) : AppDataSource {
    ...
    override fun delBill(id: UUID) {
        execute {
            ...
            val bill = billDao.getBill(id) ?: return@execute
    ...
    override fun updateBill(bill: Bill) {
        execute {
            ...
            val old = billDao.getBill(bill.uuid) ?: return@execute
    ...
    override fun getType(uuid: UUID, callBack: (Type?) -> Unit) {
        execute {
    ...
    override fun getTypes(isExpense: Boolean, callBack: (List<Type>?) -> Unit) {
        execute {
    ...
    fun getTypesOnBackground(isExpense: Boolean, callBack: (List<Type>?) -> Unit) {
        execute {
